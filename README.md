 ...
 * In particular, when optimizing Binaryen uses an additional IR while writing the wasm binary, Stack IR (see `src/wasm-stack.h`). Stack IR allows a bunch of optimizations that are tailored for the stack machine form of WebAssembly binary format. Since those optimizations are only done while optimizing, a round-trip operation *without* optimization may slightly increase the size of the wasm, if it was particularly well-optimized before. TODO: opt level?
