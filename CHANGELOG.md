...
-------------
 ...
 - `wasm-split`'s `--multi-split` mode now supports more options:
     ...
     `--emit-text`, and `--placeholdermap`. Because `--no-placeholders` is false
     by default and until now `--multi-split` didn't use placeholders at all,
     this is a breaking change. If you want to continue to do multi-split
     without placeholders, you need to explicitly specify `--no-placeholders`.
