...
TIDY_MSG=$(git diff -U0 $GITHUB_BASE_REF...$GITHUB_HEAD_REF | $CLANG_TIDY_DIFF -quiet -p1 2> /dev/null)
if [ -n "$TIDY_MSG" -a "$TIDY_MSG" != "No relevant changes found." ]; then
  ...
  git diff -U0 $GITHUB_BASE_REF...$GITHUB_HEAD_REF | $CLANG_TIDY_DIFF -quiet -p1 2> /dev/null
  exit 1
fi
