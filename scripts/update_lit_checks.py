...
NOTICE = f';; NOTE: Assertions have been generated by {script_name}'
...
def find_run_lines(test, lines):
    matches = [m.group(1)
               for m in [RUN_LINE_RE.match(l) for l in lines]
               if m]
    if not matches:
        ...
        for line in lines:
            ...
            if func_match:
                for prefix, funcs in func_dict.items():
                    body = funcs.get(func_match.group(1), [])
                    for l in body:
                        output_lines.append(f' ;; {prefix}:{l}')
