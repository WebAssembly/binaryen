...
private:
  ...
  std::vector<RuntimeTable> definedTables;
    ...
    for (auto& table : wasm.tables) {
      if (table->imported()) {
        ...
        auto& runtimeTable = definedTables.emplace_back(
          *table, [this](std::string_view s) { externalInterface->trap(s); });
        auto [_, inserted] = allTables.try_emplace(table->name, &runtimeTable);
        (void)inserted; // for noassert builds
        assert(inserted && "Unexpected repeated table name");
        ...
        if (table->type.isNullable()) {
          ...
          // TODO?
          if (!runtimeTable.grow(table->initial, null)) {
            externalInterface->trap("Failed to create initial table");
          }
        }
      }
    }
