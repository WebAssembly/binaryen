...
public:
  ...
  void clearIf(Name target) {
    if (breakTo == target) { breakTo.clear(); }
  }
...
public:
  ...
  Flow visitLoop(Loop* curr) {
    ...
    while (1) {
      ...
      return flow; // loop does not loop automatically, only continue achieves
                   // that
    }
  }
...
public:
  ...
  Literal callFunctionInternal(Name name, const LiteralList& arguments) {
    ...
    assert(
      !flow.breaking() ||
      flow.breakTo ==
        RETURN_FLOW); // cannot still be breaking, it means we missed our stop
    ...
    callDepth = previousCallDepth; // may decrease more than one, if we jumped
                                   // up the stack
