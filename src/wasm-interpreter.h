...
template <typename SubType> class ExpressionRunner : public Visitor<SubType, Flow> {
public:
  ...
  Flow visitUnary(Unary* curr) {
    ...
    if (value.type == i32) {
      switch (curr->op) {
        ...
        case EqZInt32:
          return Literal(int32_t(value == Literal(int32_t(0))));
        ...
        case ConvertUInt32ToFloat32:
          return value.convertUToF32();
        case ConvertUInt32ToFloat64:
          return value.convertUToF64();
        case ConvertSInt32ToFloat32:
          return value.convertSToF32();
        case ConvertSInt32ToFloat64:
          return value.convertSToF64();
        case ExtendS8Int32:
          return Literal(int32_t(int8_t(value.geti32() & 0xFF)));
        case ExtendS16Int32:
          return Literal(int32_t(int16_t(value.geti32() & 0xFFFF)));
        default:
          WASM_UNREACHABLE();
      }
    }
    if (value.type == i64) {
      switch (curr->op) {
        ...
        case PopcntInt64:
          return value.popCount();
        case EqZInt64:
          return Literal(int32_t(value == Literal(int64_t(0))));
        case WrapInt64:
        ...
        case ConvertSInt64ToFloat64:
          return value.convertSToF64();
        case ExtendS8Int64:
          return Literal(int64_t(int8_t(value.geti64() & 0xFF)));
        case ExtendS16Int64:
          return Literal(int64_t(int16_t(value.geti64() & 0xFFFF)));
        case ExtendS32Int64:
          return Literal(int64_t(int32_t(value.geti64() & 0xFFFFFFFF)));
        default:
          WASM_UNREACHABLE();
      }
    }
  ...
  Flow visitBinary(Binary* curr) {
    ...
    if (left.type == i32) {
      switch (curr->op) {
        ...
        case ShlInt32:
          return left.shl(right.and_(Literal(int32_t(31))));
        case ShrUInt32:
          return left.shrU(right.and_(Literal(int32_t(31))));
        case ShrSInt32:
          return left.shrS(right.and_(Literal(int32_t(31))));
        case RotLInt32:
      ...
      switch (curr->op) {
        ...
        case XorInt64:
          return left.xor_(right);
        case ShlInt64:
          return left.shl(right.and_(Literal(int64_t(63))));
        case ShrUInt64:
          return left.shrU(right.and_(Literal(int64_t(63))));
        case ShrSInt64:
          return left.shrS(right.and_(Literal(int64_t(63))));
        case RotLInt64:
          return left.rotL(right);
        case RotRInt64:
          return left.rotR(right);
...
public:
  ...
  struct ExternalInterface {
    ...
    virtual Literal callTable(
      Index index, LiteralList& arguments, Type result, SubType& instance) = 0;
  ...
  ModuleInstanceBase(Module& wasm, ExternalInterface* externalInterface)
    ...
    for (auto& global : wasm.globals) {
      globals[global->name] =
        ConstantExpressionRunner<GlobalManager>(globals).visit(global->init).value;
    }
...
public:
  ...
  Literal callFunction(Name name, LiteralList& arguments) {
    // if the last call ended in a jump up the stack, it might have left stuff for us to clean up
    // here
    callDepth = 0;
    functionStack.clear();
  ...
  Literal callFunctionInternal(Name name, LiteralList& arguments) {
    ...
    public:
      ...
      FunctionScope(Function* function, LiteralList& arguments) : function(function) {
        if (function->params.size() != arguments.size()) {
          std::cerr << "Function `" << function->name << "` expects " << function->params.size()
                    << " parameters, got " << arguments.size() << " arguments." << std::endl;
          WASM_UNREACHABLE();
        }
          ...
          if (i < arguments.size()) {
            ...
            if (function->params[i] != arguments[i].type) {
              std::cerr << "Function `" << function->name << "` expects type "
                        << printType(function->params[i]) << " for parameter " << i << ", got "
    ...
    assert(!flow.breaking() ||
           flow.breakTo == RETURN_FLOW); // cannot still be breaking, it means we missed our stop
