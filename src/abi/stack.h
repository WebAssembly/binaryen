...
inline Index stackAlign(Index size) { return (size + StackAlign - 1) & -StackAlign; }
...
inline void getStackSpace(Index local, Function* func, Index size, Module& wasm) {
  ...
  block->list.push_back(
    builder.makeSetLocal(local, builder.makeGetGlobal(stackPointer->name, PointerType)));
  // TODO: add stack max check
  Expression* added;
  if (PointerType == i32) {
    added = builder.makeBinary(AddInt32, builder.makeGetLocal(local, PointerType),
  ...
  auto makeStackRestore = [&]() {
    return builder.makeSetGlobal(stackPointer->name, builder.makeGetLocal(local, PointerType));
  };
  ...
  for (auto** ptr : finder.list) {
    ...
    if (ret->value && ret->value->type != unreachable) {
      ...
      block->list.push_back(builder.makeReturn(builder.makeGetLocal(temp, ret->value->type)));
      block->finalize();
      *ptr = block;
    } else {
      // restore, then return
