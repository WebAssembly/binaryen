...
struct Walker : public VisitorType {
  ...
  Expression* replaceCurrent(Expression* expression) {
    ...
    if (currFunction) {
      ...
      // Early exit if there is no debug info at all, and also leave if we
      // already have debug info on the new expression (which we don't want to
      // trample; if there is no debug info we want to copy, as a replacement
      // operation suggests they new code plays the same role, and has the same
      // origin; but if the code is already annotated, trust that).
