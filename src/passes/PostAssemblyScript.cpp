...
// A version of LocalGraph that considers only assignments
// when computing influences.
struct AssignmentGraph : wasm::LocalGraph {
  AssignmentGraph(Function* func) : wasm::LocalGraph(func) {}
...
// Does a cheap traversal first, remembering preconditions, and
// goes all-in only if it finds ARC code.
struct OptimizeARC : public WalkerPass<PostWalker<OptimizeARC>> {
  ...
  void visitCall(Call* curr) {
    if (curr->target == RELEASE && curr->operands.size() == 1) {
      ...
      } else if (auto* call = operand->dynCast<Call>()) {
        if (call->target == RETAIN && call->operands.size() == 1) {
          // trivial __release(__retain(...))
          // shouldn't actually be emitted, but to be sure
          Builder builder(*getModule());
          replaceCurrent(builder.makeDrop(call->operands[0]));
        }
      }
    }
  }
  ...
  // Tests if a released value has balanced retains
  bool testBalancedRetains(LocalGet* released, AssignmentGraph& graph) {
    bool atLeastOne = false;
    for (auto* localSet : graph.getSetses[released]) {
      if (localSet == nullptr) {
        return false;
      }
  ...
  void doWalkFunction(Function* func) {
    if (func->name == RETAIN || func->name == RELEASE) {
      return;
    }
    retains.clear();
    releases.clear();
    ...
    for (auto pair : retains) {
      auto* set = pair.first;
      auto** setLocation = pair.second;
      if (!testReachesReturn(set, graph)) {
        if (!testRetainsAllocation(set->value->cast<Call>()->operands[0],
                                   graph)) {
          std::unordered_set<Expression**> found;
          collectReleases(set, graph, found);
          if (!found.empty()) {
            bool allBalanced = true;
            for (auto** getLocation : found) {
              if (!testBalancedRetains(
                    (*getLocation)->cast<Call>()->operands[0]->cast<LocalGet>(),
                    graph)) {
                allBalanced = false;
                break;
              }
            ...
            if (allBalanced) {
              ...
              WasmPrinter::printExpression(set, std::cerr, true);
              std::cerr << " reaching\n";
#endif
              redundantRetains.insert(setLocation);
              for (auto** getLocation : found) {
#ifdef POST_ASSEMBLYSCRIPT_DEBUG
                std::cerr << "    ";
