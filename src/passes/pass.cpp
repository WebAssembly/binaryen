...
void PassRunner::run() {
  ...
  if (!isNested && (options.debug || passDebug)) {
    ...
    if (passDebug >= 3) { dumpWast("before", wasm); }
    for (auto* pass : passes) {
      ...
      if (passDebug == 2) { WasmPrinter::printModule(wasm, moduleBefore); }
      // prepare to run
      ...
      if (passDebug >= 3) { dumpWast(pass->name, wasm); }
    }
...
struct AfterEffectModuleChecker {
  ...
  void check() {
    if (beganWithAnyStackIR && hasAnyStackIR()) {
      ...
      if (checkers.size() != module->functions.size()) { error(); }
      for (Index i = 0; i < checkers.size(); i++) {
        ...
        // Did a name change?
        if (module->functions[i]->name != checkers[i].name) { error(); }
      }
