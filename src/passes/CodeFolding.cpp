...
struct CodeFolding : public WalkerPass<ControlFlowWalker<CodeFolding>> {
  ...
  struct Tail {
    ...
    void validate() const {
      if (expr && block) { assert(block->list.back() == expr); }
    }
  };
  ...
  void visitBlock(Block* curr) {
    ...
    if (isConcreteType(curr->list.back()->type)) { return; }
    ...
    for (auto* child : curr->list) {
      if (child->type == unreachable) { hasFallthrough = false; }
    }
    if (hasFallthrough) { tails.push_back({Tail(curr)}); }
  ...
  void doWalkFunction(Function* func) {
    ...
    while (anotherPass) {
      ...
      if (anotherPass) { ReFinalize().walkFunctionInModule(func, getModule()); }
    }
  }
...
private:
  ...
  void optimizeExpressionTails(std::vector<Tail>& tails, T* curr) {
    ...
    for (auto& tail : tails) {
      ...
      if (!tail.isFallthrough()) { tail.block->list.push_back(last); }
      ...
      for (auto& tail : next) {
        ...
        while (items.size() >= 2) {
          ...
          if (items.size() >= 2) {
            ...
            if (optimizeTerminatingTails(explore, num + 1)) { return true; }
          }
          items.swap(others);
        }
      }
