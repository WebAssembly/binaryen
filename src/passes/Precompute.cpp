...
// Execute an expression by itself. Errors if we hit anything we need anything not in the expression
// itself standalone.
...
public:
  ...
  Flow visitLoop(Loop* curr) {
    ...
    // but we can't tell if non-infinite, since we don't have state, so loops are just impossible to
    // optimize for now
    return Flow(NONSTANDALONE_FLOW);
  }

  Flow visitCall(Call* curr) { return Flow(NONSTANDALONE_FLOW); }
  Flow visitCallImport(CallImport* curr) { return Flow(NONSTANDALONE_FLOW); }
  Flow visitCallIndirect(CallIndirect* curr) { return Flow(NONSTANDALONE_FLOW); }
  ...
  Flow visitSetLocal(SetLocal* curr) { return Flow(NONSTANDALONE_FLOW); }
  Flow visitGetGlobal(GetGlobal* curr) { return Flow(NONSTANDALONE_FLOW); }
  Flow visitSetGlobal(SetGlobal* curr) { return Flow(NONSTANDALONE_FLOW); }
  ...
  Flow visitAtomicCmpxchg(AtomicCmpxchg* curr) { return Flow(NONSTANDALONE_FLOW); }
  Flow visitAtomicWait(AtomicWait* curr) { return Flow(NONSTANDALONE_FLOW); }
  Flow visitAtomicWake(AtomicWake* curr) { return Flow(NONSTANDALONE_FLOW); }
...
struct Precompute
  : public WalkerPass<PostWalker<Precompute, UnifiedExpressionVisitor<Precompute>>> {
    ...
    if (flow.breaking()) {
      ...
      if (flow.breakTo == RETURN_FLOW) {
        ...
        if (auto* ret = curr->dynCast<Return>()) {
          ...
          replaceCurrent(
            builder.makeReturn(flow.value.type != none ? builder.makeConst(flow.value) : nullptr));
        }
        return;
      }
      ...
      if (auto* br = curr->dynCast<Break>()) {
        ...
        replaceCurrent(builder.makeBreak(
          flow.breakTo, flow.value.type != none ? builder.makeConst(flow.value) : nullptr));
      }
      return;
    }
...
private:
  ...
  void optimizeLocals(Function* func) {
    ...
    std::unordered_map<SetLocal*, Literal>
      setValues; // the constant value, or none if not a constant
