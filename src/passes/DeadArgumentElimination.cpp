...
struct DAEScanner
  ...
  void doWalkFunction(Function* func) {
    ...
    if (numParams > 0 && !info->hasUnseenCalls) { findUnusedParams(func); }
  }
  ...
  void findUnusedParams(Function* func) {
    ...
    while (!work.empty()) {
      ...
      for (auto i : indexes) {
        ...
        if (iter != localUses.end()) {
          ...
          if (use == DAEBlockInfo::Read) { usedParams.insert(i); }
    ...
    for (Index i = 0; i < numParams; i++) {
      if (usedParams.count(i) == 0) { info->unusedParams.insert(i); }
    }
  }
};
...
struct DAE : public Pass {
  ...
  void run(PassRunner* runner, Module* module) override {
    ...
    while (1) {
      if (!iteration(runner, module)) { break; }
    }
  }
  ...
  bool iteration(PassRunner* runner, Module* module) {
    ...
    if (changed.empty()) {
      for (auto& func : module->functions) {
        if (func->result == none) { continue; }
        auto name = func->name;
        if (infoMap[name].hasUnseenCalls) { continue; }
        auto iter = allCalls.find(name);
        if (iter == allCalls.end()) { continue; }
  ...
  removeReturnValue(Function* func, std::vector<Call*>& calls, Module* module) {
    ...
    for (auto* call : calls) {
      ...
      *location = call;
      ...
      if (call->type != unreachable) { call->type = none; }
    }
  }
};
