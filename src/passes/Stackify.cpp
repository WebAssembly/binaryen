...
// Utility class for replacing an expression's stack value children with Pops
...
struct Stackifier : BinaryenIRWriter<Stackifier> {
  ...
  Stackifier(Function* func, Module* module);
...
Stackifier::Stackifier(Function* func, Module* module)
  : BinaryenIRWriter<Stackifier>(func), module(module),
    builder(*module, IRProfile::Stacky) {
...
void Stackifier::emitLocalGet(LocalGet* curr) {
  ...
  if (curr->type.isMulti()) {
    const auto& types = func->getLocalType(curr->index).expand();
...
void Stackifier::emitGlobalGet(GlobalGet* curr) {
  auto& instrs = scopeStack.back().instrs;
  if (curr->type.isMulti()) {
    const auto& types = module->getGlobal(curr->name)->type.expand();
