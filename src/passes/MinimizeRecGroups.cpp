...
// the amount of "wasted" work spent generating permutations that have already
// been used with the amount of work spent trying to avoid the wasted work and
// with the quality of the output.
//
// The simplest possible design that avoids all "wasted" work would be to
// canonicalize the type of each group to eagerly detect isomorphisms and
// eagerly organize the groups into equivalence classes. This would allow us to
// avoid ever generating permutations of a group that have already been used.
// See `getCanonicalPermutation` below for details on how this works. However,
// this is not an ideal solution because canonicalization is an expensive
// operation and in the common case a group will not conflict with any other
...
// symmetry can have very many automorphic permutations, so many permutations
// might need to be generated before finding one that is distinct from previous
// permutations. This work can be sidestepped by instead eagerly advancing the
// brand type, but that increases the code size of the output.
//
...
// classes of groups without canonicalizing like in the second design, but we
// canonicalize the shape for any equivalence class that grows larger than one
// group like in the first design. This ensures that we don't spend time
// canonicalizing in the common case where there are no conflicts, but it also
// ensures that we don't waste time generating automorphisms of previous groups.
// It also allows us to avoid compromising on the quality of the output.
...
struct MinimizeRecGroups : Pass {
  ...
  void updateShape(size_t group) {
    ...
    //
    //     A. It is the same shape as some group in an existing nontrivial
    //        equivalence class.
    //
    //     B. It is the same shape as some other group also not yet affiliated
    //        with a nontrivial equivalence class, so we have a new nontrivial
    //        equivalence class.
    //
