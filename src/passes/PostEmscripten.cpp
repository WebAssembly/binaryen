...
#define SAFE_MAX 1024
  ...
  void optimizeMemoryAccess(Expression*& ptr, Address& offset) {
    while (1) {
      ...
      // that much (precompute would optimize that)
      if (left) {
    ...
    // finally ptr may be a const, but it isn't worth folding that in (we still have a const); in
    // fact, it's better to do the opposite for gzip purposes as well as for readability.
  ...
  void visitStore(Store* curr) { optimizeMemoryAccess(curr->ptr, curr->offset); }
      ...
      if (import->base == POW) {
        if (auto* exponent = curr->operands[1]->dynCast<Const>()) {
          if (exponent->value == Literal(double(2.0))) {
            ...
            replaceCurrent(builder.makeBinary(MulFloat64, localizer.expr,
