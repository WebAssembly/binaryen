...
FloatTrap::FloatTrap(Mode mode)
    : mode(mode),
      didAddF64ToI64JSImport(false) {
  name = "binaryen-trap-mode";
}

void FloatTrap::visitUnary(Unary* curr) {
  replaceCurrent(makeTrappingUnary(curr));
}

void FloatTrap::visitBinary(Binary* curr) {
  replaceCurrent(makeTrappingBinary(curr));
}

void FloatTrap::visitModule(Module* curr) {
...
Expression* FloatTrap::makeTrappingBinary(Binary* curr) {
...
void FloatTrap::ensureBinaryFunc(Binary* curr) {
  Name name = getBinaryFuncName(curr);
...
void FloatTrap::ensureUnaryFunc(Unary *curr) {
  Name name = getUnaryFuncName(curr);
  if (generatedFunctions.count(name) != 0) {
    return;
  }
...
Name FloatTrap::getBinaryFuncName(Binary* curr) {
  switch (curr->op) {
...
Name FloatTrap::getUnaryFuncName(Unary* curr) {
  switch (curr->op) {
