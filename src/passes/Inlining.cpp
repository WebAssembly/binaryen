...
struct FunctionInfoScanner : public WalkerPass<PostWalker<FunctionInfoScanner>> {
  ...
  FunctionInfoScanner* create() override { return new FunctionInfoScanner(infos); }
  ...
  void visitFunction(Function* curr) { (*infos)[curr->name].size = Measurer::measure(curr->body); }
...
struct InliningState {
  ...
  std::unordered_map<Name, std::vector<InliningAction>>
    actionsForFunction; // function name => actions that can be performed in it
};
...
struct Planner : public WalkerPass<PostWalker<Planner>> {
  ...
  void visitCall(Call* curr) {
    ...
    if (state->worthInlining.count(curr->target) && curr->type != unreachable &&
      ...
      assert(state->actionsForFunction.count(getFunction()->name) >
...
static Expression* doInlining(Module* module, Function* into, InliningAction& action) {
  ...
  *action.callSite = block;
  ...
  struct Updater : public PostWalker<Updater> {
    ...
    void visitGetLocal(GetLocal* curr) { curr->index = localMapping[curr->index]; }
    void visitSetLocal(SetLocal* curr) { curr->index = localMapping[curr->index]; }
  } updater;
  ...
  for (Index i = 0; i < from->vars.size(); i++) {
    block->list.push_back(builder.makeSetLocal(updater.localMapping[from->getVarIndexBase() + i],
  ...
  bool iteration(PassRunner* runner, Module* module) {
    ...
    funcs.erase(std::remove_if(funcs.begin(), funcs.end(),
                  [&](const std::unique_ptr<Function>& curr) {
                    ...
                    bool canRemove = inlinedUses.count(name) && inlinedUses[name] == info.calls &&
                                     !info.usedGlobally;
#ifdef INLINING_DEBUG
                    if (canRemove)
