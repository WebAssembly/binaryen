...
static Name printableLocal(Index index, Function* func) {
  Name name;
  if (func) { name = func->getLocalNameOrDefault(index); }
  if (!name.is()) { name = Name::fromInt(index); }
  return name;
}
...
struct PrintExpressionContents : public Visitor<PrintExpressionContents> {
  ...
  void visitLoad(Load* curr) {
    ...
    if (curr->offset) { o << " offset=" << curr->offset; }
    if (curr->align != curr->bytes) { o << " align=" << curr->align; }
  }
...
struct PrintSExpression : public Visitor<PrintSExpression> {
  ...
  void printDebugLocation(const Function::DebugLocation& location) {
    if (lastPrintedLocation == location) { return; }
  ...
  void printDebugLocation(Expression* curr) {
    if (currFunction) {
      ...
      if (iter != debugLocations.end()) { printDebugLocation(iter->second); }
    }
  }
  ...
  void printFullLine(Expression* expression) {
    ...
    if (full) { o << "[" << printType(expression->type) << "] "; }
    visit(expression);
    o << maybeNewLine;
  }
  ...
  void visitBreak(Break* curr) {
    ...
    if (curr->condition) { printFullLine(curr->condition); }
    decIndent();
  }
  void visitSwitch(Switch* curr) {
    o << '(';
  ...
  void visitDefinedFunction(Function* curr) {
    ...
    if (curr->type.is()) { o << maybeSpace << "(type " << curr->type << ')'; }
    if (curr->params.size() > 0) {
