...
struct PrintSExpression : public Visitor<PrintSExpression> {
  ...
  void visitIf(If* curr) {
    ...
    if (!full && curr->ifTrue->is<Block>() && curr->ifTrue->dynCast<Block>()->name.isNull() &&
    ...
    if (curr->ifFalse) {
      if (!full && curr->ifFalse->is<Block>() && curr->ifFalse->dynCast<Block>()->name.isNull() &&
  ...
  template <typename CallBase> void printCallBody(CallBase* curr) {
  ...
  void visitUnary(Unary* curr) {
    ...
    switch (curr->op) {
      ...
      case ReinterpretFloat32:
        o << "i32.reinterpret/f32";
        break;
      case ReinterpretFloat64:
        o << "i64.reinterpret/f64";
        break;
      ...
      case ReinterpretInt32:
        o << "f32.reinterpret/i32";
        break;
      case ReinterpretInt64:
        o << "f64.reinterpret/i64";
        break;
      case ExtendS8Int32:
        o << "i32.extend8_s";
        break;
  ...
  void visitHost(Host* curr) {
    switch (curr->op) {
      case PageSize:
        printOpening(o, "pagesize") << ')';
        break;
      case CurrentMemory:
        printOpening(o, "current_memory") << ')';
        break;
      case GrowMemory: {
      ...
      case HasFeature:
        printOpening(o, "hasfeature ") << curr->nameOperand << ')';
        break;
      default:
        abort();
    }
  }
  void visitNop(Nop* curr) { printMinorOpening(o, "nop") << ')'; }
  void visitUnreachable(Unreachable* curr) { printMinorOpening(o, "unreachable") << ')'; }
  ...
  void visitImport(Import* curr) {
    ...
    switch (curr->kind) {
      ...
      case ExternalKind::Global:
        o << "(global " << curr->name << ' ' << printType(curr->globalType) << ")";
        break;
      default:
        WASM_UNREACHABLE();
    }
  ...
  void visitFunction(Function* curr) {
    ...
    if (curr->params.size() > 0) {
      for (size_t i = 0; i < curr->params.size(); i++) {
        o << maybeSpace;
        printMinorOpening(o, "param ")
          << printableLocal(i) << ' ' << printType(curr->getLocalType(i)) << ')';
      }
    }
    if (curr->result != none) {
      o << maybeSpace;
...
std::ostream& WasmPrinter::printExpression(
  Expression* expression, std::ostream& o, bool minify, bool full) {
  if (!expression) {
    o << "(null expression)";
    return o;
  }
