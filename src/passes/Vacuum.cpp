...
struct Vacuum : public WalkerPass<ExpressionStackWalker<Vacuum>> {
  ...
  Expression* optimize(Expression* curr, bool resultUsed, bool typeMatters) {
    ...
    auto* prev = curr;
    while (1) {
      if (typeMatters && curr->type != type) { return prev; }
      prev = curr;
      switch (curr->_id) {
        ...
        case Expression::Id::SelectId: {
          ...
          if (auto* unary = curr->dynCast<Unary>()) {
            ...
            if (tester.hasSideEffects()) { return curr; }
      ...
      if (!optimized) {
        ...
        if (list[z - skip]->type == unreachable && z < size - 1) {
          for (Index i = z - skip + 1; i < list.size(); i++) {
            auto* remove = list[i];
            if (remove) { typeUpdater.noteRecursiveRemoval(remove); }
          }
  ...
  void visitIf(If* curr) {
    ...
    if (auto* value = curr->condition->dynCast<Const>()) {
      ...
      if (value->value.getInteger()) {
        child = curr->ifTrue;
        if (curr->ifFalse) { typeUpdater.noteRecursiveRemoval(curr->ifFalse); }
      } else {
        if (curr->ifFalse) {
          child = curr->ifFalse;
      ...
      if (isConcreteType(last->type) && block->type == last->type) {
        ...
        if (!last) {
          ...
          if (block->name.is()) {
            ...
            seeker.walk(temp);
            if (seeker.found && seeker.valueType != none) { canPop = false; }
          }
          if (canPop) {
