...
struct RemoveUnusedBrs : public WalkerPass<PostWalker<RemoveUnusedBrs>> {
  ...
  static void visitAny(RemoveUnusedBrs* self, Expression** currp) {
    ...
    if (curr->is<Break>()) {
      ...
      if (name.is()) {
        ...
        if (skip > 0) { flows.resize(size - skip); }
    ...
    if (curr->targets.size() == 0) {
      ...
      if ((curr->targets.size() >= MIN_SHRINK && shrink) ||
        ...
        for (Index i = 1; i < curr->targets.size() - 1; i++) {
          if (curr->targets[i] != curr->default_) { return; }
        }
  ...
  bool optimizeLoop(Loop* loop) {
    ...
    while (1) {
      ...
      if (EffectAnalyzer(getPassOptions(), curr).branches) { return false; }
      if (i == 0) return false;
      i--;
    }
  }
  ...
  void doWalkFunction(Function* func) {
    ...
    do {
      ...
      // sink blocks
      if (sinkBlocks(func)) { anotherCycle = true; }
    } while (anotherCycle);
    ...
    struct FinalOptimizer : public PostWalker<FinalOptimizer> {
      ...
      void visitIf(If* curr) {
        ...
        if (auto* select = selectify(curr)) { replaceCurrent(select); }
      }
            ...
            if (one->type == unreachable && two->type != unreachable) {
              if (auto* br = one->dynCast<Break>()) {
                if (ExpressionAnalyzer::isSimple(br)) {
                  // Wonderful, do it!
                  Builder builder(*getModule());
                  if (flipCondition) { builder.flip(iff); }
      ...
      bool optimizeSetIfWithCopyArm(Expression** currp) {
        ...
        if (get && get->index == set->index) {
          ...
          if (get && get->index != set->index) { get = nullptr; }
        }
        if (!get) return false;
        // We can do it!
        bool tee = set->isTee();
