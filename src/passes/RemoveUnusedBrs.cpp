        ...
        if (brIf->condition && !brIf->value && brIf->name != loop->name) {
          if (i == list.size() - 2) {
            ...
            if (brIf->name == block->name &&
              ...
              // we are right.
              //   "guess #17: true", then on one arm "guess #7 was true" and
              //   "false" on the other; match them upp (ok if guess vanishes -
              //   hints can be eliminated)
              // maybe: turn hints into asserts? not for fuzzing, but testing.
              // if hint is 1, assert the value is not 0, etc.
              block->finalize();
              return true;
            }
          }
