...
struct RemoveUnusedBrs : public WalkerPass<PostWalker<RemoveUnusedBrs>> {
  ...
  static void clear(RemoveUnusedBrs* self, Expression** currp) { self->flows.clear(); }
    ...
    while (1) {
      ...
      if (auto* iff = curr->dynCast<If>()) {
        ...
        if (!iff->ifFalse) {
          ...
          if (iff->ifTrue->type == unreachable) {
            iff->ifFalse =
              blockifyMerge(iff->ifFalse, builder.stealSlice(block, i + 1, list.size()));
            iff->finalize();
            block->finalize();
            return true;
        ...
        if (brIf->condition && !brIf->value && brIf->name != loop->name) {
          if (i == list.size() - 2) {
            ...
            if (brIf->name == block->name &&
                BranchUtils::BranchSeeker::countNamed(block, block->name) == 1) {
              // note that we could drop the last element here, it is a br we know for sure is
              // removable, but telling stealSlice to steal all to the end is more efficient, it can
              // just truncate.
              list[i] = builder.makeIf(brIf->condition, builder.makeBreak(brIf->name),
  ...
  void doWalkFunction(Function* func) {
    ...
    if (worked) {
      // Our work may alter block and if types, they may now return values that we made flow through
      // them
    ...
    struct JumpThreader : public ControlFlowWalker<JumpThreader> {
      ...
      void visitBlock(Block* curr) {
        ...
        if (list.size() == 1 && curr->name.is()) {
          // if this block has just one child, a sub-block, then jumps to the former are jumps to
          // us, really
          if (auto* child = list[0]->dynCast<Block>()) {
            ...
            if (child->name.is() && child->name != curr->name && child->type == curr->type) {
              ...
              for (auto* br : breaks) {
                ...
                breaksToBlock[curr].push_back(
                  br); // update the list - we may push it even more later
              }
              breaksToBlock.erase(child);
            }
          }
        } else if (list.size() == 2) {
          // if this block has two children, a child-block and a simple jump, then jumps to
          // child-block can be replaced with jumps to the new target
          ...
          if (child && child->name.is() && jump && ExpressionAnalyzer::isSimple(jump)) {
            ...
            // if the jump is to another block then we can update the list, and maybe push it even
            // more later
    ...
    struct FinalOptimizer : public PostWalker<FinalOptimizer> {
      ...
      void visitBlock(Block* curr) {
        ...
        // * note that if not in a block already, then we need to create a block for this, so not
        // useful otherwise
        // * note that this only happens at the end of a block, as code after the if is dead
        // * note that we do this at the end, because un-conditionalizing can interfere with
        // optimizeLoop()ing.
        auto& list = curr->list;
        for (Index i = 0; i < list.size(); i++) {
          ...
          if (ifTrueBreak && !ifTrueBreak->condition &&
              canTurnIfIntoBrIf(iff->condition, ifTrueBreak->value, passOptions)) {
          ...
          if (ifFalseBreak && !ifFalseBreak->condition &&
              canTurnIfIntoBrIf(iff->condition, ifFalseBreak->value, passOptions)) {
          ...
          if (curr->name.is()) {
            ...
            if (br && br->condition && br->name == curr->name && br->type != unreachable) {
              ...
              if (BranchUtils::BranchSeeker::countNamed(curr, curr->name) == 1) {
                ...
                Builder builder(*getModule());
                replaceCurrent(builder.makeIf(builder.makeUnary(EqZInt32, br->condition), curr));
                curr->name = Name();
      ...
      void tablify(Block* block) {
        ...
        while (start < list.size() - 1) {
          ...
          while (end < list.size() && ExpressionAnalyzer::equal(
          ...
          if (num >= 2 && num >= MIN_NUM) {
            ...
            if (range <= MAX_RANGE && range <= num * NUM_TO_RANGE_FACTOR) {
              ...
              for (Index i = start; i < end; i++) {
                ...
                assert(
                  table[index] == defaultName); // we should have made sure there are no overlaps
                table[index] = name;
              }
              Builder builder(*getModule());
              ...
              if (min != 0) {
                conditionValue = builder.makeBinary(
                  SubInt32, conditionValue, builder.makeConst(Literal(int32_t(min))));
              }
              list[end - 1] = builder.makeBlock(
                defaultName, builder.makeSwitch(table, defaultName, conditionValue));
