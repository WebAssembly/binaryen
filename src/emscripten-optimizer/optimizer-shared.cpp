...
IString SIMD_INT8X16_CHECK("SIMD_Int8x16_check"), SIMD_INT16X8_CHECK("SIMD_Int16x8_check"),
  SIMD_INT32X4_CHECK("SIMD_Int32x4_check"), SIMD_FLOAT32X4_CHECK("SIMD_Float32x4_check"),
...
AsmType detectType(
  Ref node, AsmData* asmData, bool inVarDef, IString minifiedFround, bool allowI64) {
  if (node->isString()) {
    if (asmData) {
...
Ref makeAsmCoercedZero(AsmType type) {
  switch (type) {
    ...
    case ASM_FLOAT32X4: {
      return ValueBuilder::makeCall(SIMD_FLOAT32X4, ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0));
      break;
    }
    case ASM_FLOAT64X2: {
    ...
    case ASM_INT8X16: {
      return ValueBuilder::makeCall(SIMD_INT8X16, ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0),
    ...
    case ASM_INT16X8: {
      return ValueBuilder::makeCall(SIMD_INT16X8, ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0),
    ...
    case ASM_INT32X4: {
      return ValueBuilder::makeCall(SIMD_INT32X4, ValueBuilder::makeNum(0),
        ValueBuilder::makeNum(0), ValueBuilder::makeNum(0), ValueBuilder::makeNum(0));
      break;
    }
    default:
...
Ref makeAsmCoercion(Ref node, AsmType type) {
  switch (type) {
    ...
    case ASM_NONE:
    default:
      return node; // non-validating code, emit nothing XXX this is dangerous, we should only allow
                   // this when we know we are not validating
  }
}
