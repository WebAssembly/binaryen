...
struct Ref {
  ...
  bool operator==(double d) {
    abort();
    return false;
  } // prevent Ref == number, which is potentially ambiguous; use ->getNumber() == number
...
struct JSPrinter {
  ...
  JSPrinter(bool pretty_, bool finalize_, Ref ast_)
    : pretty(pretty_), finalize(finalize_), buffer(0), size(0), used(0), indent(0),
      possibleSpace(false), ast(ast_) {}
  ...
  bool isBlock(Ref node) { return node->isArray() && node[0] == BLOCK; }
...
#define BUFFERSIZE 1000
    static char full_storage_f[BUFFERSIZE],
      full_storage_e[BUFFERSIZE]; // f is normal, e is scientific for float, x for integer
    static char *storage_f = full_storage_f + 1,
                *storage_e = full_storage_e + 1; // full has one more char, for a possible '-'
    ...
    for (int e = 0; e <= 1; e++) {
      ...
      if (!integer) {
        ...
        for (int i = 0; i <= 18; i++) {
          ...
          // errv("%.18f, %.18e   =>   %s   =>   %.18f, %.18e   (%d), ", d, d, buffer, temp, temp,
          // temp == d);
          if (temp == d) {
            break;
}
        ...
        if (wasm::isUInteger64(d)) {
          ...
          snprintf(buffer, BUFFERSIZE - 1, e ? "%e" : "%.0f",
            d); // even on integers, e with a dot is useful, e.g. 1.2e+200
          sscanf(buffer, "%lf", &temp);
        }
      ...
      if (dot) {
        ...
        char* test = end;
        // remove zeros, and also doubles can use at most 24 digits, we can truncate any extras even
        // if not zero
    ...
    // fprintf(stderr, "options:\n%s\n%s\n (first? %d)\n", storage_e, storage_f, strlen(storage_e) <
    // strlen(storage_f));
    char* ret;
    if (err_e == err_f) {
  ...
  void printNum(Ref node) { emit(numToString(node->getNumber(), finalize)); }
  ...
  int getPrecedence(Ref node, bool parent) {
    ...
    if (type == BINARY || type == UNARY_PREFIX) {
      return OperatorClass::getPrecedence(
        type == BINARY ? OperatorClass::Binary : OperatorClass::Prefix, node[1]->getIString());
    } else if (type == SEQ) {
  ...
  // @param childPosition -1 means it is printed to the left of parent, 0 means "anywhere", 1 means
  // right
  ...
  void printUnaryPrefix(Ref node) {
    if (finalize && node[1] == PLUS &&
        (node[2]->isNumber() || (node[2]->isArray() && node[2][0] == UNARY_PREFIX &&
  ...
  void printIf(Ref node) {
    ...
    emit(')');
    space();
    // special case: we need braces to save us from ambiguity, if () { if () } else. otherwise else
    // binds to inner if also need to recurse for                                if () { if () { }
    // else { if () } else (note that this is only a problem if the if body has a single element in
    // it, not a block or such, as then the block would be braced) this analysis is a little
    // conservative - it assumes any child if could be confused with us, which implies all other
    // braces vanished (the worst case for us, we are not saved by other braces).
...
class ValueBuilder {
  static Ref makeRawString(const IString& s) { return &arena.alloc<Value>()->setString(s); }

  static Ref makeNull() { return &arena.alloc<Value>()->setNull(); }

public:
  static Ref makeRawArray(int size_hint = 0) { return &arena.alloc<Value>()->setArray(size_hint); }

  static Ref makeToplevel() {
    return &makeRawArray(2)->push_back(makeRawString(TOPLEVEL)).push_back(makeRawArray());
  }

  static Ref makeString(IString str) {
    return &makeRawArray(2)->push_back(makeRawString(STRING)).push_back(makeRawString(str));
  }

  static Ref makeBlock() {
    return &makeRawArray(2)->push_back(makeRawString(BLOCK)).push_back(makeRawArray());
  }
  ...
  static Ref makeCall(Ref target, Ref arg) {
    Ref ret =
      &makeRawArray(3)->push_back(makeRawString(CALL)).push_back(target).push_back(makeRawArray());
    ret[2]->push_back(arg);
    return ret;
  }
  ...
  static Ref makeDouble(double num) { return &arena.alloc<Value>()->setNumber(num); }
  ...
  static Ref makeBinary(Ref left, IString op, Ref right) {
    if (op == SET) {
      ...
      return &makeRawArray(3)->push_back(makeRawString(SEQ)).push_back(left).push_back(right);
    } else {
  ...
  static Ref makeVar(bool is_const = false) {
    return &makeRawArray(2)->push_back(makeRawString(VAR)).push_back(makeRawArray());
  }
  ...
  static Ref makeIndexing(Ref target, Ref index) {
    return &makeRawArray(3)->push_back(makeRawString(SUB)).push_back(target).push_back(index);
  }
  ...
  static Ref makeDo(Ref body, Ref condition) {
    return &makeRawArray(3)->push_back(makeRawString(DO)).push_back(condition).push_back(body);
  }

  static Ref makeWhile(Ref condition, Ref body) {
    return &makeRawArray(3)->push_back(makeRawString(WHILE)).push_back(condition).push_back(body);
  }
  ...
  static void appendCaseToSwitch(Ref switch_, Ref arg) {
    ...
    switch_[2]->push_back(&makeRawArray(2)->push_back(arg).push_back(makeRawArray()));
  }

  static void appendDefaultToSwitch(Ref switch_) {
    assert(switch_[0] == SWITCH);
    switch_[2]->push_back(&makeRawArray(2)->push_back(makeNull()).push_back(makeRawArray()));
  }
  ...
  static Ref makeArray() {
    return &makeRawArray(2)->push_back(makeRawString(ARRAY)).push_back(makeRawArray());
  }
  ...
  static Ref makeObject() {
    return &makeRawArray(2)->push_back(makeRawString(OBJECT)).push_back(makeRawArray());
  }

  static void appendToObject(Ref array, IString key, Ref value) {
    assert(array[0] == OBJECT);
    array[1]->push_back(&makeRawArray(2)->push_back(makeRawString(key)).push_back(value));
  }

  static Ref makeSub(Ref obj, Ref index) {
    return &makeRawArray(2)->push_back(makeRawString(SUB)).push_back(obj).push_back(index);
  }

  static Ref makePtrShift(Ref ptr, int shifts) { return makeBinary(ptr, RSHIFT, makeInt(shifts)); }
};

// Tolerates 0.0 in the input; does not trust a +() to be there.
class DotZeroValueBuilder : public ValueBuilder {
public:
  static Ref makeDouble(double num) { return makePrefix(PLUS, ValueBuilder::makeDouble(num)); }
};
