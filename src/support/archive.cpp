...
Archive::Archive(Buffer& b, bool& error)
  : data(b), symbolTable({nullptr, 0}), stringTable({nullptr, 0}), firstRegularData(nullptr) {
  error = false;
  if (data.size() < strlen(magic) || memcmp(data.data(), magic, strlen(magic))) {
    error = true;
    return;
  }
...
Archive::SubBuffer Archive::Child::getBuffer() const { return {data + startOfFile, getSize()}; }

std::string Archive::Child::getRawName() const { return getHeader()->getName(); }
...
std::string Archive::Child::getName() const {
  ...
  if (name[0] == '/') {
    ...
    std::string addr(
      parent->stringTable.data + offset, parent->stringTable.data + parent->stringTable.len);
...
struct Symbol {
  ...
  void next(Archive::SubBuffer& symbolTable) {
    ...
    stringIndex = strchr((char*)symbolTable.data + stringIndex, '\0') - (char*)symbolTable.data + 1;
    ++symbolIndex;
  }
};
} // namespace
...
void Archive::dump() const {
  printf(
    "Archive data %p len %zu, firstRegularData %p\n", data.data(), data.size(), firstRegularData);
