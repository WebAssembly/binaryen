...
T wasm::read_file(
  ...
  if (uint64_t(insize) >= std::numeric_limits<size_t>::max()) {
    // Building a 32-bit executable where size_t == 32 bits, we are not able to create strings
    // larger than 2^32 bytes in length, so must abort here.
    std::cerr << "Failed opening '" << filename << "': Input file too large: " << insize
  ...
  if (binary == Flags::Text) {
    ...
    input.resize(chars + 1); // Truncate size to the number of ASCII characters actually read in
                             // text mode (which is generally less than the number of bytes on
...
template std::vector<char> wasm::read_file<>(
  const std::string&, Flags::BinaryOption, Flags::DebugOption);
