...
private:
  ...
  // to ensure validation in the wasm spec. Note that the binary reader code
  // skips such extra casts where possible, which avoids roundtrips increasing
  // code size continuously. The wasm spec will hopefully improve to use the
  // more refined type as well, which would remove the need for this hack.
  //
  ...
  // We also need locals for entire tuples at a time, for br_if tuples (unlike
  // tuple.extract which only needs a single local each time, for the extracted
  // lane). Each index in this map indicates the initial index of a series of
  // locals, one for each lane in the tuple.
