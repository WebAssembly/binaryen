  ...
  if (in.takeSExprStart("v128.const"sv)) {
    ...
    if (in.takeKeyword("i16x8"sv)) {
      for (int i = 0; i < 8; i++) {
        ...
        // ???
        lanes.lanes.push_back(Literal(static_cast<int32_t>(*int_)));
      }
      ...
      for (int i = 0; i < 4; i++) {
        ...
        lanes.lanes.push_back(Literal(*int_));
      }
    } else if (in.takeKeyword("i8x16"sv)) {
      for (int i = 0; i < 16; i++) {
        auto int_ = in.takeI8();
        if (!int_) {
      ...
      for (int i = 0; i < 4; ++i) {
        if (auto f = in.takeF32()) {
          lanes.lanes.push_back(Literal(*f));
        } else {
          auto kind = nan(in);
          CHECK_ERR(kind);
          lanes.lanes.push_back(NaNResult{*kind, Type::f32});
        }
      }
      ...
      for (int i = 0; i < 2; ++i) {
        if (auto f = in.takeF64()) {
          ...
          auto kind = nan(in);
          CHECK_ERR(kind);
          lanes.lanes.push_back(NaNResult{*kind, Type::f64});
        }
      }
    } else {
