...
IString ASM_FUNC("asmFunc"), ABORT_FUNC("abort"),
  FUNCTION_TABLE("FUNCTION_TABLE"),
  NO_RESULT("wasm2js$noresult"), // no result at all
  EXPRESSION_RESULT(
    "wasm2js$expresult"); // result in an expression, no temp var
...
template<typename T> static uint64_t constOffset(const T& segment) {
...
public:
  ...
  IString fromName(Name name, NameScope scope) {
    ...
    if (it != mangledScope.end()) { return it->second; }
    ...
    for (int i = 0;; i++) {
      ...
      if (!allMangledNames.count(ret)) { break; }
      ...
      // the
      //     global-most scope, but rather in the "asmFunc" scope which is
...
Ref Wasm2JSBuilder::processWasm(Module* wasm, Name funcName) {
  ...
  tableSize =
    std::accumulate(wasm->table.segments.begin(),
                    wasm->table.segments.end(),
                    0,
                    [&](size_t size, Table::Segment seg) -> size_t {
...
void Wasm2JSBuilder::addFunctionImport(Ref ast, Function* import) {
  ...
  // below.
  if (ABI::wasm2js::isScratchMemoryHelper(import->base)) { return; }
  Ref theVar = ValueBuilder::makeVar();
  ast->push_back(theVar);
  Ref module =
    ValueBuilder::makeName(ENV); // TODO: handle nested module imports
  ValueBuilder::appendToVar(
    theVar,
...
void Wasm2JSBuilder::addGlobalImport(Ref ast, Global* import) {
  ...
  Ref value =
    ValueBuilder::makeDot(module, fromName(import->base, NameScope::Top));
  if (import->type == i32) { value = makeAsmCoercion(value, ASM_INT); }
  ValueBuilder::appendToVar(
...
void Wasm2JSBuilder::addExports(Ref ast, Module* wasm) {
  ...
  for (auto& export_ : wasm->exports) {
    ...
    if (export_->kind == ExternalKind::Memory) {
      ...
      ValueBuilder::appendToObject(descs, IString("grow"), growDesc);
      ValueBuilder::appendToObject(
      ...
      ValueBuilder::appendToObject(bufferDesc, IString("get"), bufferGetter);
      ValueBuilder::appendToObject(descs, IString("buffer"), bufferDesc);
...
Ref Wasm2JSBuilder::processFunction(Module* m,
  ...
  if (theVar[1]->size() == 0) { ret[3]->splice(theVarIndex, 1); }
  // checks
  assert(frees[i32].size() ==
         temps[i32]); // all temp vars should be free at the end
  assert(frees[f32].size() ==
         temps[f32]); // all temp vars should be free at the end
  assert(frees[f64].size() ==
         temps[f64]); // all temp vars should be free at the end
  return ret;
}
...
Ref Wasm2JSBuilder::processFunctionBody(Module* m, Function* func) {
  struct ExpressionProcessor : public Visitor<ExpressionProcessor, Ref> {
    ...
    struct ScopedTemp {
      ...
      // it has already been
      //                 allocated in a higher scope, and we can just assign to
      //                 it as our result is going there anyhow.
      ...
      ~ScopedTemp() {
        if (needFree) { parent->freeTemp(type, temp); }
      }
    ...
    Ref visit(Expression* curr, IString nextResult) {
      ...
      result = old; // keep it consistent for the rest of this frame, which may
                    // call visit on multiple children
      return ret;
    }
    ...
    Ref visitIf(If* curr) {
      ...
      Ref ifFalse;
      if (curr->ifFalse) { ifFalse = visit(curr->ifFalse, NO_RESULT); }
    ...
    Ref visitCall(Call* curr) {
      ...
      for (auto operand : curr->operands) {
        theCall[2]->push_back(makeAsmCoercion(visit(operand, EXPRESSION_RESULT),
    ...
    Ref visitBinary(Binary* curr) {
      ...
      switch (curr->type) {
          ...
          switch (curr->op) {
            ...
            case GeUInt32:
              return ValueBuilder::makeBinary(makeSigning(left, ASM_UNSIGNED),
                                              ...
                                              makeSigning(right, ASM_UNSIGNED));
            case EqFloat32:
            case EqFloat64: return ValueBuilder::makeBinary(left, EQ, right);
            case NeFloat32:
            case NeFloat64: return ValueBuilder::makeBinary(left, NE, right);
            case GeFloat32:
            case GeFloat64: return ValueBuilder::makeBinary(left, GE, right);
            case GtFloat32:
            case GtFloat64: return ValueBuilder::makeBinary(left, GT, right);
            case LeFloat32:
            case LeFloat64: return ValueBuilder::makeBinary(left, LE, right);
            case LtFloat32:
            case LtFloat64: return ValueBuilder::makeBinary(left, LT, right);
            case RotLInt32:
            case RotRInt32:
        ...
        case f64:
          ...
          if (curr->type == f32) { return makeAsmCoercion(ret, ASM_FLOAT); }
          return ret;
        default:
    ...
    Ref visitSelect(Select* curr) {
      ...
      ScopedTemp tempIfTrue(curr->type, parent, func),
        tempIfFalse(curr->type, parent, func), tempCondition(i32, parent, func);
    ...
    Ref visitReturn(Return* curr) {
      if (!curr->value) { return ValueBuilder::makeReturn(Ref()); }
      Ref val = makeAsmCoercion(visit(curr->value, EXPRESSION_RESULT),
...
void Wasm2JSGlue::emitMemory(std::string buffer, std::string segmentWriter) {
  ...
  for (auto& seg : wasm.memory.segments) {
    ...
    out << segmentWriter << "(" << constOffset(seg) << ", \""
