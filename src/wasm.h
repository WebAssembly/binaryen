...
class Export {
public:
  Name name;  // exported name - note that this is the key, as the internal name
              // is non-unique (can have multiple exports for an internal, also
              // over kinds)
...
private:
  ...
  std::map<Name, Export*>
    exportsMap; // exports map is by the *exported* name, which is unique
