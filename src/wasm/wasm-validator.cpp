...
struct FunctionValidator : public WalkerPass<PostWalker<FunctionValidator>> {
  ...
  std::unordered_set<Name>
    labelNames; // Binaryen IR requires that label names must be unique - IR
...
void FunctionValidator::noteBreak(Name name,
  ...
  if (!info.hasBeenSet()) {
    ...
    if (arity != info.arity) { info.arity = BreakInfo::PoisonArity; }
  }
}
...
static bool checkOffset(Expression* curr, Address add, Address max) {
  ...
  if (raw > std::numeric_limits<Address::address_t>::max()) { return false; }
...
bool WasmValidator::validate(Module& module, Flags flags) {
  ...
  if (PassRunner::getPassDebug()) { validateBinaryenIR(module, info); }
  // print all the data
