...
void WasmBinaryWriter::writeResizableLimits(
  Address initial, Address maximum, bool hasMaximum, bool shared, bool is64, bool hasPageSize, Address::address32_t pageSizeLog2) {
  ...
  for (size_t i = 0; i < num; i++) {
    ...
    switch (kind) {
      ...
      case ExternalKind::Table: {
        ...
        table->type = getType();
        bool is_shared;
        Address::address32_t page_size = -1;
...
void WasmBinaryReader::readTableDeclarations() {
  ...
  for (size_t i = 0; i < num; i++) {
    ...
    table->hasExplicitName = isExplicit;
    bool is_shared;
    Address::address32_t pageSize = -1;
    getResizableLimits(table->initial,
                       table->max,
                       is_shared,
                       table->addressType,
