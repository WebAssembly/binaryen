...
struct FixInvokeFunctionNamesWalker : public PostWalker<FixInvokeFunctionNamesWalker> {
  ...
  void visitImport(Import* curr) {
    ...
    if (newname == curr->name) {
      // No rename needed for this import
      imports.insert(curr->name);
      return;
    }
    ...
    if (imports.insert(newname).second == false) {
      // An invoke function with the correct signature altready exists.
      // Mark this (now duplicate) import for deletion.
      toRemove.push_back(curr->name);
    } else {
      // Rename this import to the new name
      curr->base = newname;
      curr->name = newname;
    }
  }
