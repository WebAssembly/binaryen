...
struct Asm2WasmPreProcessor {
  ...
  char* process(char* input) {
    ...
    if (marker) { *marker = START_FUNCS[0]; }
    ...
    if (debugInfo) {
      ...
      if (out >= end) { Fatal() << "error in handling debug info"; }
      *out = 0;
      input = copy;
    }
...
static Call* checkDebugInfo(Expression* curr) {
  if (auto* call = curr->dynCast<Call>()) {
    if (call->target == EMSCRIPTEN_DEBUGINFO) { return call; }
  }
  return nullptr;
}
...
private:
  void allocateGlobal(IString name, Type type, Literal value = Literal()) {
    ...
    if (value.type == none) { value = Literal::makeZero(type); }
  ...
  Type getResultTypeOfCallUsingParent(Ref parent, AsmData* data) {
    ...
    if (!!parent) {
      ...
      if (parent[0] != SEQ) { result = detectWasmType(parent, data); }
    }
    return result;
  }
...
private:
  AsmType detectAsmType(Ref ast, AsmData* data) {
    if (ast->isString()) {
      ...
      if (view != views.end()) { return view->second.type; }
    }
...
void Asm2WasmBuilder::processAsm(Ref ast) {
  ...
  if (ast[1]->size() == 0) { Fatal() << "empty input"; }
  ...
  struct FinalizeCalls : public WalkerPass<PostWalker<FinalizeCalls>> {
    ...
    void visitCall(Call* curr) {
      ...
      if (calledFunc && !calledFunc->imported()) {
        ...
        if (curr->type != result) { curr->type = result; }
  ...
  if (preprocessor.debugInfo) { wasm.removeFunction(EMSCRIPTEN_DEBUGINFO); }
  ...
  std::function<Expression*(Ref)> process = [&](Ref ast) -> Expression* {
    ...
    if (what == BINARY) {
      ...
      if (ast[1] == PLUS) {
        ...
        if (literal.type != none) { return builder.makeConst(literal); }
        ...
        if (ret->type == f32) { return ensureDouble(ret); }
        fixCallType(ret, f64);
        return ret;
      } else if (ast[1] == MINUS) {
        if (ast[2]->isNumber() ||
            ...
            if (readType != ASM_NONE) {
              if (readValue->isArray(SUB) && readValue[1]->isString() &&
                ...
                if (writeType == ASM_INT &&
                  ...
                  conv->type = Type::f32;
                  if (readType == ASM_DOUBLE) { return ensureDouble(conv); }
                  return conv;
      ...
      if (canSwitch) {
        ...
        // ensure a default
        if (br->default_.isNull()) { br->default_ = top->name; }
        if (breakWhenNotMatching) { breakWhenNotMatching->name = br->default_; }
