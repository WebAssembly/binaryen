...
BinaryenExpressionRef BinaryenLoop(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenLoop", StringLit(name), body); }
...
BinaryenExpressionRef BinaryenGetLocal(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenGetLocal", index, type); }
...
BinaryenExpressionRef BinaryenSetLocal(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenSetLocal", index, value); }
...
BinaryenExpressionRef BinaryenTeeLocal(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenTeeLocal", index, value); }
...
BinaryenExpressionRef BinaryenConst(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenConst", value); }
  return static_cast<Expression*>(ret);
}
BinaryenExpressionRef BinaryenUnary(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenUnary", op, value); }
...
BinaryenExpressionRef BinaryenBinary(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenBinary", op, left, right); }
...
BinaryenExpressionRef BinaryenDrop(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenDrop", value); }
...
BinaryenExpressionRef BinaryenReturn(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenReturn", value); }
...
BinaryenExpressionRef BinaryenNop(BinaryenModuleRef module) {
  ...
  if (tracing) { traceExpression(ret, "BinaryenNop"); }
...
BinaryenExpressionRef BinaryenUnreachable(BinaryenModuleRef module) {
  ...
  if (tracing) { traceExpression(ret, "BinaryenUnreachable"); }
...
BinaryenExpressionRef BinaryenSIMDShift(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenSIMDShift", op, vec, shift); }
  return static_cast<Expression*>(ret);
}
...
BinaryenExpressionRef BinaryenDataDrop(BinaryenModuleRef module,
  ...
  if (tracing) { traceExpression(ret, "BinaryenDataDrop", segment); }
  return static_cast<Expression*>(ret);
}
...
BinaryenModuleRef BinaryenModuleParse(const char* text) {
  if (tracing) { std::cout << "  // BinaryenModuleRead\n"; }
...
void BinaryenModulePrint(BinaryenModuleRef module) {
  if (tracing) { std::cout << "  BinaryenModulePrint(the_module);\n"; }

  WasmPrinter::printModule((Module*)module);
}

void BinaryenModulePrintAsmjs(BinaryenModuleRef module) {
  if (tracing) { std::cout << "  BinaryenModulePrintAsmjs(the_module);\n"; }
...
int BinaryenModuleValidate(BinaryenModuleRef module) {
  if (tracing) { std::cout << "  BinaryenModuleValidate(the_module);\n"; }
...
void BinaryenModuleOptimize(BinaryenModuleRef module) {
  if (tracing) { std::cout << "  BinaryenModuleOptimize(the_module);\n"; }
...
int BinaryenGetOptimizeLevel(void) {
  if (tracing) { std::cout << "  BinaryenGetOptimizeLevel();\n"; }
...
void BinaryenModuleAutoDrop(BinaryenModuleRef module) {
  if (tracing) { std::cout << "  BinaryenModuleAutoDrop(the_module);\n"; }
...
static BinaryenBufferSizes writeModule(BinaryenModuleRef module,
  ...
  if (sourceMapUrl) { writer.setSourceMap(&os, sourceMapUrl); }
  writer.write();
...
BinaryenModuleWrite(BinaryenModuleRef module, char* output, size_t outputSize) {
  if (tracing) { std::cout << "  // BinaryenModuleWrite\n"; }
...
void BinaryenModuleInterpret(BinaryenModuleRef module) {
  if (tracing) { std::cout << "  BinaryenModuleInterpret(the_module);\n"; }
...
BinaryenGetFunctionTypeBySignature(BinaryenModuleRef module,
                                   ...
                                   BinaryenType* paramTypes,
                                   BinaryenIndex numParams) {
  if (tracing) { std::cout << "  // BinaryenGetFunctionTypeBySignature\n"; }
  ...
  // Lock. Guard against reading the list while types are being added.
  {
    ...
    for (BinaryenIndex i = 0; i < wasm->functionTypes.size(); i++) {
      ...
      if (curr->structuralComparison(test)) { return curr; }
    }
  }
