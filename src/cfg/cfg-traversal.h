...
struct CFGWalker : public ControlFlowWalker<SubType, VisitorType> {
  ...
  std::vector<BasicBlock*>
    loopTops; // blocks that are the tops of loops, i.e., have backedges to them

  // traversal state
  BasicBlock*
    currBasicBlock; // the current block in play during traversal. can be nullptr if unreachable,
  ...
  static void doStartIfFalse(SubType* self, Expression** currp) {
    ...
    self->startBasicBlock();
    self->link(
      self->ifStack[self->ifStack.size() - 2], self->currBasicBlock); // before if -> ifFalse
  }

  static void doEndIf(SubType* self, Expression** currp) {
    auto* last = self->currBasicBlock;
    self->startBasicBlock();
    self->link(last, self->currBasicBlock); // last one is ifFalse's fallthrough if there was one,
                                            // otherwise it's the ifTrue fallthrough
  ...
  static void doStartLoop(SubType* self, Expression** currp) {
    auto* last = self->currBasicBlock;
    self->startBasicBlock();
    self->loopTops.push_back(
      self->currBasicBlock); // a loop with no backedges would still be counted here, but oh well
  ...
  void unlinkDeadBlocks(std::unordered_set<BasicBlock*> alive) {
    for (auto& block : basicBlocks) {
      ...
      block->in.erase(std::remove_if(block->in.begin(), block->in.end(),
                        [&alive](BasicBlock* other) { return !alive.count(other); }),
        block->in.end());
      block->out.erase(std::remove_if(block->out.begin(), block->out.end(),
                         [&alive](BasicBlock* other) { return !alive.count(other); }),
        block->out.end());
    }
  }
