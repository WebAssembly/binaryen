...
public:
  ...
  OptimizingIncrementalModuleBuilder(
    ...
    for (uint32_t i = 0; i < numWorkers;
         i++) { // TODO: one less, and add it at the very end, to not compete
                // with main thread?
      createWorker();
    }
    waitUntilAllReady();
...
private:
  ...
  void queueFunction(Function* func) {
    ...
    assert(nextFunction < numFunctions); // TODO: if we are given more than we
                                         // expected, use a slower work queue?
    ...
    for (uint32_t i = 0; i < self->numFunctions; i++) {
      ...
      if (self->list[i].load() == self->endMarker) {
        ...
        self->activeWorkers--;
        {
          ...
          if (!self->finishing) { // while waiting for the lock, things may have
                                  // ended
