...
int main(int argc, const char* argv[]) {
  ...
  if (!translateToFuzz) {
    ...
    if (fuzzPasses) { reader.pickPasses(options); }
  ...
  if (fuzzExecBefore) { results.get(wasm); }
  ...
  if (fuzzExecAfter && fuzzBinary) {
    ...
    if (options.passOptions.validate) {
      ...
      if (!valid) { WasmPrinter::printModule(&other); }
      assert(valid);
    }
    curr = &other;
  }
  ...
  if (options.runningPasses()) {
    ...
    auto runPasses = [&]() {
      ...
      if (options.passOptions.validate) {
        ...
        if (!valid) { WasmPrinter::printModule(&*curr); }
        assert(valid);
      }
    };
    runPasses();
  ...
  if (fuzzExecAfter) { results.check(*curr); }
