...
struct MetaDCEGraph {
  ...
  std::unordered_map<Name, Name>
    exportToDCENode; // export exported name => DCE name
  ...
  std::unordered_map<Name, Name>
    importIdToDCENode; // import module.base => DCE name
  ...
  void scanWebAssembly() {
    ...
    for (auto& segment : wasm.memory.segments) {
      if (!segment.isPassive) { rooter.walk(segment.offset); }
    }
...
public:
  ...
  void apply() {
    ...
    for (auto& exp : wasm.exports) {
      ...
      if (reached.find(dceName) == reached.end()) { toRemove.push_back(name); }
    }
    ...
    passRunner.add(
      "reorder-functions"); // removing functions may alter the optimum order,
                            // as # of calls can change
    passRunner.run();
  }
  ...
  void printAllUnused() {
    ...
    for (auto& pair : nodes) {
      ...
      if (reached.find(name) == reached.end()) { unused.insert(name.str); }
    }
...
int main(int argc, const char* argv[]) {
  ...
  if (graphFile.size() == 0) { Fatal() << "no graph file provided."; }
  ...
  const json::IString NAME("name"), REACHES("reaches"), ROOT("root"),
