...
void Instrumenter::addSecondaryMemory(size_t numFuncs) {
  ...
  // The memory uses the default page size to avoid issues in case custom page sizes are not supported
  size_t pages = (numFuncs + Memory::kDefaultPageSize - 1) / Memory::kDefaultPageSize;
...
void Instrumenter::addProfileExport(size_t numFuncs) {
  ...
  if (wasm->memories.empty()) {
    size_t pages = (profileSize + Memory::kDefaultPageSize - 1) / Memory::kDefaultPageSize;
    wasm->addMemory(Builder::makeMemory("0"));
    wasm->memories[0]->initial = pages;
    wasm->memories[0]->max = pages;
  } else{
    size_t pages = (profileSize + (1<<wasm->memories[0]->pageSizelog2) - 1) / (1<<wasm->memories[0]->pageSizelog2);
