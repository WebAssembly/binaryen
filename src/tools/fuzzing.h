...
public:
  ...
  void pickPasses(OptimizationOptions& options) {
    ...
    if (oneIn(2)) { options.passOptions.optimizeLevel = upTo(4); }
    if (oneIn(2)) { options.passOptions.shrinkLevel = upTo(4); }
  ...
  void build() {
    ...
    if (HANG_LIMIT > 0) { addHangLimitSupport(); }
    if (!allowNaNs) { addDeNanSupport(); }
    finalizeTable();
  }
...
private:
  ...
  bool finishedInput; // whether we already cycled through all the input (if so,
                      // we should try to finish things off)
  ...
  void readData(std::vector<char> input) {
    ...
    if (bytes.size() == 0) { bytes.push_back(0); }
  }
  ...
  Function* addFunction() {
    ...
    if (oneIn(10)) { bodyType = unreachable; }
    ...
    if (HANG_LIMIT > 0) { addHangLimitChecks(func); }
    assert(breakableStack.empty());
    assert(hangStack.empty());
    wasm.addFunction(func);
  ...
  void recombine(Function* func) {
    ...
    for (auto& pair : scanner.exprsByType) {
      ...
      if (trimmed.empty()) { trimmed.push_back(vectorPick(list)); }
      list.swap(trimmed);
    }
  ...
  void addInvocations(Function* func) {
    ...
    while (oneIn(2) && !finishedInput) {
      ...
      if (isConcreteType(func->result)) { invoke = builder.makeDrop(invoke); }
      invocations.push_back(invoke);
      // log out memory in some cases
      if (oneIn(2)) { invocations.push_back(makeMemoryHashLogging()); }
    }
  ...
  Expression* makeTrivial(Type type) {
    ...
    assert(type == unreachable);
    Expression* ret = nullptr;
    if (isConcreteType(func->result)) { ret = makeTrivial(func->result); }
  ...
  Expression* makeLoop(Type type) {
    ...
    if (oneIn(2)) {
      ...
      list.push_back(builder.makeBreak(
        ret->name, nullptr, makeCondition())); // possible branch back
  ...
  Expression* makeCondition() {
    ...
    auto* ret = make(i32);
    if (oneIn(2)) { ret = builder.makeUnary(UnaryOp::EqZInt32, ret); }
    return ret;
  }
  ...
  Expression* makeBreak(Type type) {
    ...
    if (type != unreachable) { hangStack.pop_back(); }
    return makeTrivial(type);
  }
  ...
  Expression* makeNonAtomicLoad(Type type) {
    ...
    switch (type) {
      ...
      case v128: {
        if (!wasm.features.hasSIMD()) { return makeTrivial(type); }
        return builder.makeLoad(
  ...
  Expression* makeNonAtomicStore(Type type) {
    ...
    // type.
    if (type == none) { type = getConcreteType(); }
    auto offset = logify(get());
    auto ptr = makePointer();
    auto value = make(type);
    switch (type) {
  ...
  Literal makeArbitraryLiteral(Type type) {
    ...
    switch (upTo(4)) {
      ...
      case 3: {
        ...
        // maybe negative
        if (oneIn(2)) { value = value.mul(Literal::makeFromInt32(-1, type)); }
        return value;
      }
    }
    WASM_UNREACHABLE();
  ...
  Literal makeLiteral(Type type) {
    ...
    if (!allowNaNs && ret.isNaN()) { ret = Literal::makeFromInt32(0, type); }
    return ret;
  }
  ...
  Expression* makeSIMD(Type type) {
    assert(wasm.features.hasSIMD());
    if (type != v128) { return makeSIMDExtract(type); }
    switch (upTo(6)) {
