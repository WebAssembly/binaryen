...
//
// Allows iteration over the immediate fields of the expression.
//

class ImmediateIterator {
...
bool
Address
AtomicRMWOp, SIMDExtractOp, SIMDReplaceOp, SIMDShiftOp
Literal
UnaryOp, BinaryOp
  };
  ...
  struct Iterator {
    const ImmediateIterator& parent;
    Index index;

    Iterator(const ImmediateIterator& parent, Index index) : parent(parent), index(index) {}
...
public:
  ...
  ImmediateIterator(Expression* parent) {
    struct Traverser : public PostWalker<Traverser> {
      Expression* parent;
      std::vector<Expression*>* children;
