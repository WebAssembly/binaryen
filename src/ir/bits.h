  ...
  static Expression* makeSignExt(Expression* value, Index bytes, Module& wasm) {
    if (value->type == i32) {
      if (bytes == 1 || bytes == 2) {
        ...
        return builder.makeBinary(ShrSInt32,
          builder.makeBinary(ShlInt32, value, LiteralUtils::makeFromInt32(shifts, i32, wasm)),
      ...
      if (bytes == 1 || bytes == 2 || bytes == 4) {
        ...
        Builder builder(wasm);
        return builder.makeBinary(ShrSInt64,
          builder.makeBinary(ShlInt64, value, LiteralUtils::makeFromInt32(shifts, i64, wasm)),
