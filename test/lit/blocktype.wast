...
(module
 ...
 ;; CHECK-NEXT:  (loop $block (type $f2) (result (ref $f2) (ref $f1))
 ;; CHECK-NEXT:   (br $block)
 ;; CHECK-NEXT:  )
 ;; CHECK-NEXT: )
 ;; RTRIP:      (func $f2 (type $f2) (result (ref $f2) (ref $f1))
 ;; RTRIP-NEXT:  (loop $label (type $f2) (result (ref $f2) (ref $f1))
 ;; RTRIP-NEXT:   (br $label)
 ;; RTRIP-NEXT:  )
 ;; RTRIP-NEXT: )
 (func $f2 (type $f2) (result (ref $f2) (ref $f1))
  ;; This block will be emitted with type $f2
  (loop $block (type $block2) (result (ref $f2) (ref $f1))
   (br $block)
  )
 )
 ...
 ;; CHECK-NEXT: )
 ;; RTRIP:      (func $f3 (type $2) (result i32)
 ;; RTRIP-NEXT:  (i32.const 0)
 ;; RTRIP-NEXT: )
 (func $f3 (result i32)
  ;; Type index 4 is the implicit (func (result i32)) from $f3's signature,
  ;; unless $block in $f2 was parsed as a block keyword and created a
  ;; spurious () -> () implicit type at index 4.
  (block (type 4) (result i32)
   (i32.const 0)
  )
 )
)
