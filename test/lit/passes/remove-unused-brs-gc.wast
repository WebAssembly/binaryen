...
(module
 ...
 (func $if-tests (param $x (ref $struct)) (param $y (ref $struct)) (result i32)
  ;; An if with a test in one arm (and also in a condition). We turn this into
  ;; a select, as avoiding the control flow is worth the unconditional test.
  (if (result i32)
 ...
 (func $if-tests-arms (param $x (ref $struct)) (param $y (ref $struct)) (param $z (ref $struct)) (result i32)
  ;; As above but now with a test in both arms. We still optimize.
  (if (result i32)
   (ref.test (ref $substruct)
    (local.get $x)
   )
