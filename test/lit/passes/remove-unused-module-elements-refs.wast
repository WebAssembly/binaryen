...
(module
  ...
  (func $target-B (type $B)
    ...
    ;; suitable CallRef. However, since we are assuming an open world, we cannot
    ;; rule out that the function reference escapes and is called by the
    ;; outside, so we must keep the function unmodified.
  )
)
