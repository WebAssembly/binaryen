;; Engine-level test of the jspi-hooks pass: the wrapped module runs under
;; JSPI with the harness in jspi-hooks.js, which prints the event trace the
;; module's own hook implementations log. This module gets the
;; standardized try_table wrappers; jspi-hooks-legacy.wast is the same module
;; with a legacy try, so it gets the legacy wrappers.
