...
;; RUN: wasm-split --instrument -all %s -o %t.instrumented.wasm

;; Generate a profile
;; RUN: node --experimental-wasm-threads %S/call_exports.mjs %t.instrumented.wasm %t.prof

;; Attempt to split the instrumented module
;; RUN: not wasm-split %t.instrumented.wasm -all --profile=%t.prof -o1 %t.1.wasm -o2 %t.2.wasm \
;; RUN:   2>&1 | filecheck %s
