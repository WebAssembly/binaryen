...
TEST(DeltaDebuggingTest, SingleItem) {
  ...
  DeltaDebugger<int> dd(items);
  while (!dd.finished()) {
    dd.resolve(std::find(dd.test.begin(), dd.test.end(), 3) != dd.test.end());
  }
...
TEST(DeltaDebuggingTest, MultipleItemsAdjacent) {
  ...
  DeltaDebugger<int> dd(items);
  while (!dd.finished()) {
    bool has2 = std::find(dd.test.begin(), dd.test.end(), 2) != dd.test.end();
    bool has3 = std::find(dd.test.begin(), dd.test.end(), 3) != dd.test.end();
...
TEST(DeltaDebuggingTest, MultipleItemsNonAdjacent) {
  ...
  while (!dd.finished()) {
    bool has2 = std::find(dd.test.begin(), dd.test.end(), 2) != dd.test.end();
    bool has5 = std::find(dd.test.begin(), dd.test.end(), 5) != dd.test.end();
...
TEST(DeltaDebuggingTest, DifferentTypes) {
  ...
  while (!dd.finished()) {
    bool hasBanana =
      std::find(dd.test.begin(), dd.test.end(), "banana") != dd.test.end();
    bool hasDate =
