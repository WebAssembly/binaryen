...
void test_autogenerated_api() {
  ...
  BinaryenExpressionRef call =
    BinaryenCallRef(
      module,
      // Give the call an unreachable target as a workaround for not being able
      // to define a Signature type in the C API yet.
      BinaryenUnreachable(module),
      NULL,
      0,
      0,
  ...
  BinaryenFunctionRef caller = BinaryenAddFunction(
    module, "foo", BinaryenTypeNone(), BinaryenTypeNone(), NULL, 0, call);
