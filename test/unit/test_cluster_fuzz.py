...
class ClusterFuzz(utils.BinaryenTestCase):
    ...
    def test_file_contents(self):
        ...
        for i in range(1, N + 1):
            ...
            assert os.path.exists(second_binary_file)
            subprocess.check_call(shared.WASM_OPT + ['-all', second_binary_file, '-q'])

            # Note its size (we leave detailed metrics for the first one; they
            # are generated by the same logic in run.py, so just verifying some
            # valid second wasms are emitted, of random sizes, is enough).
        ...
        print(f'mean sizes:   {statistics.mean(seen_second)}')
        print(f'stdev sizes:  {statistics.stdev(seen_second)}')
        print(f'median sizes: {statistics.median(seen_second)}')
