...
class StackIRTest(utils.BinaryenTestCase):
    ...
    def test_stack_ir_opts(self):
        ...
        opt = shared.run_process(shared.WASM_OPT + [path, '--generate-stack-ir', '--optimize-stack-ir', '--print', '--shrink-level=1', '-o', 'a.wasm'], capture_output=True).stdout
        nonopt = shared.run_process(shared.WASM_OPT + [path, '--generate-stack-ir', '--print', '--shrink-level=1', '-o', 'b.wasm'], capture_output=True).stdout
