...
class StackIRTest(utils.BinaryenTestCase):
    ...
    def test_stack_ir_opts(self):
        ...
        opt = shared.run_process(shared.WASM_OPT + [path, '-O', '--generate-stack-ir', '--optimize-stack-ir', '--print', '-o', 'a.wasm'], capture_output=True).stdout
        nonopt = shared.run_process(shared.WASM_OPT + [path, '-O', '--generate-stack-ir', '--print', '-o', 'b.wasm'], capture_output=True).stdout
